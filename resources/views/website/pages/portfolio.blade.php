...
@section('title', 'Portfolio — StagePass Audio Visual Limited')
@section('description', 'Explore our portfolio of professional audio visual projects, event productions, and creative solutions. See how we bring events to life with cutting-edge technology.')
@section('keywords', 'portfolio, audio visual projects, event production portfolio, stagepass portfolio, AV projects Kenya')
...
<div class="min-h-screen bg-white">
    ...
    <div class="pt-28 pb-4">
        <div class="mx-auto max-w-7xl px-6">
            @include('website.partials.breadcrumb', ['items' => [['label' => 'Portfolio', 'path' => '/portfolio']]])
        </div>
    </div>
    ...
    <section class="py-16 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
        ...
        <div class="container mx-auto px-6 lg:px-12 relative z-10">
            ...
            @if($error)
                <div class="text-center text-red-600 font-medium py-12 bg-red-50 rounded-lg border border-red-200">{{ $error }}</div>
            @endif

            @if(!$isLoading && !$error && count($instagramItems) === 0)
                <div class="text-center text-gray-600 font-medium py-12">
            ...
            @if(!$isLoading && !$error && count($instagramItems) > 0)
                <div x-data="{ isImageModalOpen: false, isVideoModalOpen: false, currentImageUrl: '', currentImageTitle: '', currentVideoUrl: '', currentVideoTitle: '' }">
                    <div class="mb-8 text-center">
                        <p class="text-gray-600">Showing <span class="font-bold text-[#172455]">{{ count($instagramItems) }}</span> posts from Instagram</p>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        @foreach($instagramItems as $index => $item)
                            @php
                            ...
                            <div class="group relative overflow-hidden rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 cursor-pointer aspect-[4/3]"
                                 ...
                                 @click="{{ $isVideo ? "currentVideoUrl = " . json_encode($mediaUrl) . "; currentVideoTitle = " . json_encode($itemTitle) . "; isVideoModalOpen = true;" : "currentImageUrl = " . json_encode($mediaUrl) . "; currentImageTitle = " . json_encode($itemTitle) . "; isImageModalOpen = true;" }}">
                                <img src="{{ $thumbnail }}" 
                                ...
                                <div class="absolute bottom-0 left-0 right-0 p-6 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                                    <h3 class="text-white font-bold text-lg line-clamp-2">{{ $itemTitle }}</h3>
