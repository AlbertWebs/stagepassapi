...
<section x-data="{ selectedIndustry: null, isModalOpen: false }" class="relative py-20 overflow-hidden" style="background: linear-gradient(160deg, #f8fafc 0%, #eef2ff 55%, #fff7ed 100%);">
    ...
    <div class="relative container mx-auto px-6 lg:px-12">
        <div class="text-center max-w-3xl mx-auto">
            ...
            <p class="text-xl text-gray-700 max-w-3xl mx-auto font-medium" style="max-width:600px;">{{ $subtitle }}</p>
        </div>
        ...
        <div class="mt-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($items as $idx => $item)
                ...
                @endphp
                <article class="ind1-card {{ $gridClass }} group relative rounded-2xl p-[1px] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-indigo-200/60 cursor-pointer focus-within:ring-2 focus-within:ring-indigo-400/60" style="animation: ind1TileDrift 6s ease-in-out infinite; animation-delay: {{ ($idx % 4) * 0.35 }}s;" @click="selectedIndustry = @js($industryForJs); isModalOpen = true" tabindex="0" role="button" @keydown.enter.prevent="selectedIndustry = @js($industryForJs); isModalOpen = true" @keydown.space.prevent="selectedIndustry = @js($industryForJs); isModalOpen = true">
                    <div class="ind1-card-inner border border-slate-200/90 bg-white/90 backdrop-blur p-6 text-center">
                        <div class="mx-auto h-12 w-12 rounded-xl bg-gradient-to-br from-indigo-950 to-indigo-800 text-white grid place-items-center text-lg transition-transform duration-300 group-hover:scale-110 ring-2 ring-yellow-300/50">{{ $idx % 3 === 0 ? '◉' : ($idx % 3 === 1 ? '◆' : '●') }}</div>
                        <h3 class="mt-4 text-base lg:text-lg font-semibold text-[#172455]">{{ $name }}</h3>
                        ...
                        <span class="absolute right-4 top-4 text-[11px] font-semibold text-slate-300">#{{ $idx + 1 }}</span>
                    </div>
                </article>
            @endforeach
        </div>
