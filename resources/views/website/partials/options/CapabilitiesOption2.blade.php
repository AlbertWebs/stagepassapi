...
<section id="services" class="relative overflow-hidden py-20 text-white">
    ...
    <div class="absolute inset-0 z-0 bg-center bg-cover bg-no-repeat" style="background-image: url('{{ asset('uploads/backgrounds/bg.webp') }}');" aria-hidden="true"></div>
    ...
    <div class="relative z-10 container mx-auto px-6 lg:px-12">
        <div class="max-w-3xl mx-auto text-center mb-12">
            ...
            <h2 class="mt-5 text-3xl lg:text-5xl font-black text-[#eaf0ff]">{{ $title }}</h2>
            <div class="mx-auto mt-4 h-1.5 w-24 rounded-full bg-gradient-to-r from-yellow-400 via-yellow-500 to-amber-500"></div>
            <p class="mt-5 text-base lg:text-lg text-slate-300">{{ $subtitle }}</p>
        </div>
        <div class="mt-2 mb-4 flex items-center justify-between gap-3">
            ...
            <div class="hidden sm:flex items-center gap-2">
                <button type="button" data-cap-prev="{{ $sliderId }}" class="h-9 w-9 rounded-full border border-slate-600 text-slate-200 hover:border-yellow-400 hover:text-yellow-300 transition-colors">←</button>
                <button type="button" data-cap-next="{{ $sliderId }}" class="h-9 w-9 rounded-full border border-slate-600 text-slate-200 hover:border-yellow-400 hover:text-yellow-300 transition-colors">→</button>
            </div>
        </div>
        <div id="{{ $sliderId }}" class="mt-4 flex gap-6 overflow-x-auto snap-x snap-mandatory pb-4 [scrollbar-width:none] [-ms-overflow-style:none]" style="-webkit-overflow-scrolling:touch;">
            @foreach($items as $idx => $item)
                ...
                <article class="cap2-card snap-start shrink-0 w-[84%] sm:w-[56%] lg:w-[34%] rounded-2xl border border-slate-700/70 bg-slate-900/75 p-7 transition-all duration-300 hover:scale-[1.02] hover:border-yellow-400/70 hover:shadow-xl hover:shadow-yellow-500/10">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[11px] font-semibold text-slate-500">0{{ $idx + 1 }}</span>
                    </div>
                    ...
                    <h3 class="mt-4 text-2xl font-bold">{{ $titleItem }}</h3>
                    <p class="mt-4 text-slate-300 text-sm lg:text-base">{{ $desc }}</p>
                    ...
                    <div class="mt-3 flex items-center justify-between text-[11px] uppercase tracking-[0.12em] text-slate-500">
                        ...
                        <span class="text-yellow-300/80">{{ $serviceMetaLabel }}</span>
                    </div>
                </article>
            @endforeach
        </div>
