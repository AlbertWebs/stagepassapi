    ...
    <div class="relative container mx-auto px-6 lg:px-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach($items as $idx => $item)
                ...
                @endphp
                <article class="cap1-card group relative rounded-2xl border border-slate-200/90 bg-white/90 backdrop-blur p-7 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-yellow-200/60" style="animation: cap1CardDrift 5.5s ease-in-out infinite; animation-delay: {{ ($idx % 3) * 0.45 }}s;">
                    <div class="absolute right-5 top-5 text-xs font-semibold text-slate-300">0{{ $idx + 1 }}</div>
                    ...
                    <h3 class="mt-5 text-xl font-bold text-[#172455]">{{ $titleItem }}</h3>
                    <p class="mt-3 text-sm lg:text-base text-slate-600">{{ $desc }}</p>
