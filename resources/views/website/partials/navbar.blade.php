...
@endphp
<div x-data="{ 
    isScrolled: {{ $isPage ? 'true' : 'false' }}, 
    isMobileMenuOpen: false, 
    activeLink: @js($currentPath),
    init() {
        if (!{{ $isPage ? 'true' : 'false' }}) {
            ...
            this.activeLink = @js($currentPath);
            this.isScrolled = true;
        }
    }
}"
    <nav :class="(isScrolled || {{ $isPage ? 'true' : 'false' }}) ? 'bg-[#0f1b3d] shadow-xl border-b-2 border-[#172455]/10' : 'bg-[#0f1b3d] backdrop-blur-md'"
