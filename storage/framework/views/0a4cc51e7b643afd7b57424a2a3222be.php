...
<html lang="en" class="h-full bg-slate-950">
    <head>
        ...
        <title><?php echo e(config('app.name', 'StagePass Admin')); ?></title>
        ...
        <div class="min-h-full flex">
            <aside class="w-72 shrink-0 border-r border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900 flex flex-col">
                ...
                <nav class="px-4 py-6 space-y-1 overflow-y-auto">
                    <?php $__currentLoopData = $navItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a
                            ...
                            class="group flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200
                                <?php echo e($activeKey === $item['key'] ? 'bg-gradient-to-r from-yellow-500/20 via-yellow-500/10 to-slate-900 text-yellow-300 shadow-lg shadow-yellow-500/10' : 'text-slate-300 hover:text-white hover:bg-slate-900/60 hover:translate-x-1'); ?>"
                        >
                            <div class="flex-1 flex items-center gap-3">
                                <div class="w-1.5 h-1.5 rounded-full <?php echo e($activeKey === $item['key'] ? 'bg-yellow-500' : 'bg-slate-600 group-hover:bg-yellow-500/50'); ?> transition-colors"></div>
                    ...
                    <div class="mt-4 pt-4 border-t border-slate-800" x-data="{ open: <?php echo e($isPageActive ? 'true' : 'false'); ?> }">
                        <button
                            type="button"
                            @click="open = !open"
                            class="w-full group flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200
                                <?php echo e($isPageActive ? 'bg-gradient-to-r from-yellow-500/20 via-yellow-500/10 to-slate-900 text-yellow-300 shadow-lg shadow-yellow-500/10' : 'text-slate-300 hover:text-white hover:bg-slate-900/60'); ?>"
                        >
                            <div class="flex items-center gap-3">
                                <div class="w-1.5 h-1.5 rounded-full <?php echo e($isPageActive ? 'bg-yellow-500' : 'bg-slate-600 group-hover:bg-yellow-500/50'); ?> transition-colors"></div>
                                <span>Pages</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-xs px-2 py-0.5 rounded-full <?php echo e($isPageActive ? 'bg-yellow-500/20 text-yellow-300' : 'bg-slate-800 text-slate-500 group-hover:bg-slate-700 group-hover:text-slate-300'); ?> transition-colors">7</span>
                                <svg 
                        ...
                        >
                            <?php $__currentLoopData = $pageItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pageItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a
                                    href="<?php echo e($pageItem['href']); ?>"
                                    class="group flex items-center gap-2 block px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200
                                        <?php echo e($activeKey === $pageItem['key'] ? 'bg-slate-900/80 text-yellow-300 shadow-md' : 'text-slate-400 hover:text-white hover:bg-slate-900/60 hover:translate-x-1'); ?>"
                                >
                                    <div class="w-1 h-1 rounded-full <?php echo e($activeKey === $pageItem['key'] ? 'bg-yellow-500' : 'bg-slate-600 group-hover:bg-yellow-500/50'); ?> transition-colors"></div>
                                    <span><?php echo e($pageItem['label']); ?></span>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    </div>
            ...
            <div class="flex-1 flex flex-col">
                <header class="flex items-center justify-between px-8 py-6 border-b border-slate-800 bg-slate-950/70 backdrop-blur">
                    <div>
                        <h1 class="text-2xl font-black text-white"><?php echo $__env->yieldContent('page_title'); ?></h1>
                        <p class="text-sm text-slate-400"><?php echo $__env->yieldContent('page_subtitle'); ?></p>
                    </div>

                    <div class="flex items-center gap-4">
                        <a href="<?php echo e(route('admin.dashboard')); ?>" class="px-4 py-2 rounded-full bg-slate-900 text-slate-200 text-sm font-semibold hover:bg-slate-800 transition">
                            Quick Actions
                        </a>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            >
                                <a href="<?php echo e(route('admin.profile')); ?>" class="block px-4 py-3 text-sm text-slate-200 hover:bg-slate-800">Profile</a>
                                <a href="<?php echo e(route('admin.settings')); ?>" class="block px-4 py-3 text-sm text-slate-200 hover:bg-slate-800">Settings</a>
                                <a href="<?php echo e(route('admin.backup')); ?>" class="block px-4 py-3 text-sm text-slate-200 hover:bg-slate-800">Backup</a>
                                <a href="<?php echo e(route('admin.maintain')); ?>" class="block px-4 py-3 text-sm text-slate-200 hover:bg-slate-800">Maintain</a>
                                <a href="<?php echo e(route('admin.email-test')); ?>" class="block px-4 py-3 text-sm text-slate-200 hover:bg-slate-800">Email Test</a>
...
<?php /**PATH C:\projects\stagepassapi\resources\views/admin/layout.blade.php ENDPATH**/ ?>