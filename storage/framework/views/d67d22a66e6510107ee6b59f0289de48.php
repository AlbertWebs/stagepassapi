<?php
    ...
    // Third priority: site_logo_url from settings (but only if footer_logo_url is not set)
    if (!$logoUrl && isset($homepageData['settings']['site_logo_url']) && empty($homepageData['settings']['footer_logo_url'])) {
    ...
    // Normalize URL - handle relative paths (only if not already a full URL)
...
<footer x-data="{ isVisible: true }" 
    ...
    <div :class="isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'"
        ...
        <div :class="isVisible ? 'animate-fade-in-up' : 'opacity-0'"
            ...
            <!-- Column 1 - Brand -->
            <div class="space-y-6">
                <img src="<?php echo e($logoUrl); ?>" alt="StagePass Logo" class="h-12 w-auto object-contain brightness-0 invert" />
                <p class="text-gray-300 text-sm leading-relaxed font-medium"><?php echo e($description); ?></p>
                <div class="flex space-x-3">
                    <?php $__currentLoopData = $socialLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e($link['url'] ?? $link->url ?? '#'); ?>" 
                           class="w-12 h-12 bg-white/10 hover:bg-gradient-to-br hover:from-yellow-400 hover:to-yellow-600 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 group">
                            <?php if(($link['platform'] ?? $link->platform ?? '') === 'Facebook'): ?>
                            ...
                            <?php elseif(($link['platform'] ?? $link->platform ?? '') === 'Twitter'): ?>
                            ...
                            <?php elseif(($link['platform'] ?? $link->platform ?? '') === 'Instagram'): ?>
                            ...
                            <?php elseif(($link['platform'] ?? $link->platform ?? '') === 'Linkedin'): ?>
                            ...
                            <?php elseif(($link['platform'] ?? $link->platform ?? '') === 'Youtube'): ?>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <?php $__currentLoopData = $quickLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li>
                            <?php if(($link['isPage'] ?? $link->isPage ?? false)): ?>
                                <a href="<?php echo e($link['href'] ?? $link->href ?? '#'); ?>" class="text-gray-300 hover:text-yellow-400 transition-colors font-medium flex items-center">
                                    <span class="mr-2">→</span> <?php echo e($link['label'] ?? $link->label ?? ''); ?>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <?php $__currentLoopData = $moreLinks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li>
                            <?php if(($link['isQuote'] ?? false)): ?>
                                <button @click="document.dispatchEvent(new CustomEvent('open-quote-modal'))" class="text-gray-300 hover:text-yellow-400 transition-colors font-medium flex items-center w-full text-left">
                                    <span class="mr-2">→</span> <?php echo e($link['label']); ?>

                                </button>
                            <?php elseif(($link['isPage'] ?? false)): ?>
                                <a href="<?php echo e($link['href']); ?>" class="text-gray-300 hover:text-yellow-400 transition-colors font-medium flex items-center">
                                    <span class="mr-2">→</span> <?php echo e($link['label']); ?>
                ...
                <ul class="space-y-3">
                    <?php $__currentLoopData = $serviceItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li class="text-gray-300 font-medium text-sm flex items-center">
                            ...
                            <?php echo e($item['label'] ?? $item->label ?? ''); ?>
        ...
        <div :class="isVisible ? 'animate-fade-in-up' : 'opacity-0'"
            ...
            <div class="text-gray-400 text-sm font-medium"><?php echo e($copyright); ?></div>
        </div>
    </div>
    ...
    <button
        ...
        class="fixed bottom-8 right-8 w-16 h-16 bg-gradient-to-br from-yellow-400 to-yellow-600 hover:from-yellow-500 hover:to-yellow-700 text-[#172455] rounded-full flex items-center justify-center shadow-2xl hover:shadow-yellow-500/50 transition-all duration-300 hover:scale-110 z-50 group animate-bounce-slow"
        style="display: none;"
        id="scrollToTopBtn"
    >
...
<?php /**PATH C:\projects\stagepassapi\resources\views/website/partials/footer.blade.php ENDPATH**/ ?>